...
export default function TeamPage() {
  ...
  const fetchUsers = async () => {
    try {
      const { data } = await api.get('/auth/me'); // This is a placeholder, need a proper users route
      // For now, let's just show the current user or mock some data if route not ready
  ...
  if (currentUser?.role !== 'Admin' && currentUser?.role !== 'Project Coordinator') {
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
          <thead>
            <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--border)' }}>
              <th style={{ padding: '1.25rem', color: 'var(--text-muted)', fontWeight: '600' }}>User</th>
              <th style={{ padding: '1.25rem', color: 'var(--text-muted)', fontWeight: '600' }}>Role</th>
              <th style={{ padding: '1.25rem', color: 'var(--text-muted)', fontWeight: '600' }}>Status</th>
          ...
          <tbody>
            {users.map((u, i) => (
              <tr key={i} style={{ borderBottom: i === users.length - 1 ? 'none' : '1px solid var(--border)' }}>
                ...
                <td style={{ padding: '1.25rem' }}>
                  <span style={{ fontSize: '0.625rem', padding: '0.25rem 0.625rem', borderRadius: '100px', background: 'rgba(16, 185, 129, 0.1)', color: '#10b981', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                    Active
                  </span>
                </td>
